...
MediaPlayer = function(aContext) {
    "use strict";
    var VERSION = "1.3.0 (refactor)", context = aContext, system, manifestLoader, abrController, element, source, streamController, rulesController, manifestUpdater, metricsExt, metricsModel, videoModel, initialized = false, playing = false, autoPlay = true, scheduleWhilePaused = false, protectionData, bufferMax = MediaPlayer.dependencies.BufferController.BUFFER_SIZE_REQUIRED, isReady = function() {
...
Dash.dependencies.DashHandler = function() {
    "use strict";
    var index = -1, requestedTime, isDynamic, type, currentTime = 0, zeroPadToLength = function(numStr, minStrLength) {
        ...
        if (isDynamic && isNaN(this.timelineConverter.getExpectedLiveEdge())) {
            var liveEdge = segments[lastIdx].presentationStartTime, metrics = this.metricsModel.getMetricsFor("stream");
        ...
        var self = this, hasInitialization = representation.initialization, hasSegments = representation.segmentInfoType !== "BaseURL" && representation.segmentInfoType !== "SegmentBase";
        ...
        var self = this, i, len, s, segments = [], count = 0, seg;
        for (i = 0, len = fragments.length; i < len; i += 1) {
            s = fragments[i];
...
Dash.dependencies.FragmentExtensions = function() {
    ...
    var parseTFDT = function(ab) {
        ...
        var self = this, request = new XMLHttpRequest(), url, loaded = false, errorStr, parsed;
        url = media;
        request.onloadend = function() {
            if (!loaded) {
...
Dash.dependencies.RepresentationController = function() {
    ...
    var data = null, dataIndex = -1, updating = true, availableRepresentations = [], currentRepresentation, updateData = function(dataValue, adaptation, type) {
        ...
        var self = this, r = representation, metrics = self.metricsModel.getMetricsFor("stream"), manifestUpdateInfo = self.metricsExt.getCurrentManifestUpdate(metrics), repInfo, alreadyAdded = false;
    ...
    }, onWallclockTimeUpdated = function(sender, isDynamic) {
    ...
    }, onBufferLevelUpdated = function(sender) {
        var streamProcessor = sender.streamProcessor, self = this, range = self.timelineConverter.calcSegmentAvailabilityRange(currentRepresentation, streamProcessor.isDynamic());
    ...
    }, onQualityChanged = function(sender, type, oldQuality, newQuality) {
        var self = this;
...
Dash.dependencies.TimelineConverter = function() {
    ...
    var clientServerTimeShift = 0, isClientServerTimeSyncCompleted = false, expectedLiveEdge = NaN, calcAvailabilityTimeFromPresentationTime = function(presentationTime, mpd, isDynamic, calculateEnd) {
        ...
        var duration = representation.segmentDuration, start = representation.adaptation.period.start, end = start + representation.adaptation.period.duration, range = {
            start: start,
            end: end
        }, checkTime, now;
...
MediaPlayer.dependencies.AbrController = function() {
    "use strict";
    var autoSwitchBitrate = true, topQualities = {}, qualityDict = {}, confidenceDict = {}, getInternalQuality = function(type) {
        var quality;
        ...
        var self = this, mediaInfo = this.adapter.convertDataToTrack(trackData).mediaInfo, type = mediaInfo.type, max;
    ...
    return {
        ...
        getPlaybackQuality: function(streamProcessor) {
            var self = this, type = streamProcessor.getType(), quality, oldQuality, rules, confidence, callback = function(res) {
        ...
        setPlaybackQuality: function(type, newPlaybackQuality) {
            var quality = getInternalQuality(type), isInt = newPlaybackQuality !== null && !isNaN(newPlaybackQuality) && newPlaybackQuality % 1 === 0;
        ...
        isPlayingAtTopQuality: function() {
            var self = this, isAtTop, audioQuality = self.getQualityFor("audio"), videoQuality = self.getQualityFor("video");
            isAtTop = audioQuality === getTopQualityIndex("audio") && videoQuality === getTopQualityIndex("video");
            return isAtTop;
        },
        reset: function() {
            var rules = this.abrRulesCollection.getRules(MediaPlayer.rules.ABRRulesCollection.prototype.QUALITY_SWITCH_RULES), rule, ln = rules.length, i = 0;
            for (i; i < ln; i += 1) {
                rule = rules[i];
...
MediaPlayer.dependencies.BufferController = function() {
    ...
    var STALL_THRESHOLD = .5, QUOTA_EXCEEDED_ERROR_CODE = 22, initializationData = [], requiredQuality = 0, currentQuality = -1, isBufferingCompleted = false, bufferLevel = 0, criticalBufferLevel = Number.POSITIVE_INFINITY, mediaSource, maxAppendedIndex = -1, lastIndex = -1, type, buffer = null, minBufferTime, hasSufficientBuffer = null, appendedBytesInfo, isBufferLevelOutrun = false, isAppendingInProgress = false, pendingMedia = [], inbandEventFound = false, waitingForInit = function() {
        ...
        var self = this, currentTime = self.playbackController.getTime(), removeStart, removeEnd, range, req;
        if (!buffer) return;
...
MediaPlayer.dependencies.FragmentController = function() {
    ...
    var fragmentModels = [], inProgress = false, findModel = function(context) {
        ...
        var self = this, bytes = self.process(response);
        if (bytes === null) {
...
MediaPlayer.dependencies.FragmentLoader = function() {
    ...
    var RETRY_ATTEMPTS = 3, RETRY_INTERVAL = 500, xhrs = [], doLoad = function(request, remainingAttempts) {
        ...
        httpRequestMetrics = self.metricsModel.addHttpRequest(request.mediaType, null, request.type, request.url, null, request.range, request.requestStartDate, null, null, null, null, request.duration);
...
MediaPlayer.dependencies.LiveEdgeFinder = function() {
    "use strict";
    var isSearchStarted = false, rules, onSearchCompleted = function(req) {
        var liveEdge = req.value, searchTime = (new Date().getTime() - this.streamProcessor.getStreamInfo().manifestInfo.loadedTime.getTime()) / 1e3;
        if (liveEdge !== null) {
...
MediaPlayer.dependencies.ManifestLoader = function() {
    ...
    var RETRY_ATTEMPTS = 3, RETRY_INTERVAL = 500, parseBaseUrl = function(url) {
        ...
        onload = function() {
            ...
            self.metricsModel.addHttpRequest("stream", null, "MPD", url, null, null, requestTime, loadedTime, request.status, null, null);
...
MediaPlayer.dependencies.Notifier = function() {
    ...
    var system, id = 0, getId = function() {
        ...
        var event, events = this.eventList;
        for (event in events) {
...
MediaPlayer.dependencies.PlaybackController = function() {
    "use strict";
    var WALLCLOCK_TIME_UPDATE_INTERVAL = 1e3, currentTime = 0, liveStartTime = NaN, wallclockTimeIntervalId, streamInfo, videoModel, trackInfo, isDynamic, getStreamStartTime = function(streamInfo) {
        ...
        var self = this, currentTime = self.getTime(), metrics = self.metricsModel.getMetricsFor(trackInfo.mediaInfo.type), DVRMetrics = self.metricsExt.getCurrentDVRInfo(metrics), DVRWindow = DVRMetrics ? DVRMetrics.range : null, actualTime;
        ...
        var currentTime = this.getTime(), actualTime = getActualPresentationTime.call(this), timeChanged = !isNaN(actualTime) && actualTime !== currentTime;
        if (timeChanged) {
...
MediaPlayer.dependencies.ProtectionExtensions.prototype = {
    ...
    getKeySystems: function(protectionData) {
        var self = this, _protectionData = {
            ...
            var decodedChallenge = null, headers = {}, parser = new DOMParser(), xmlDoc = parser.parseFromString(String.fromCharCode.apply(null, new Uint16Array(rawMessage.buffer)), "application/xml");
            ...
            var key, headerOverrides = _protectionData.licenseRequest && _protectionData.licenseRequest["com.microsoft.playready"] ? _protectionData.licenseRequest && _protectionData.licenseRequest["com.microsoft.playready"].headers : null;
            if (headerOverrides) {
                headers = headers || [];
        ...
        return [ {
            ...
            getUpdate: function(sessionId, rawMessage, laUrl, element) {
                ...
                var key, headerOverrides = _protectionData.licenseRequest && _protectionData.licenseRequest["com.widevine.alpha"] ? _protectionData.licenseRequest && _protectionData.licenseRequest["com.widevine.alpha"].headers : null;
                if (headerOverrides) {
                    for (key in headerOverrides) {
                        if ("authorization" === key.toLowerCase()) {
                            xhr.withCredentials = true;
...
MediaPlayer.models.ProtectionModel = function() {
    "use strict";
    var element = null, keyAddedListener = null, keyErrorListener = null, keyMessageListener = null, session, keySystems = [], onKeySystemUpdateCompleted = function(sender, data, error) {
        if (error) return;
        session.update(data);
    };
    return {
...
MediaPlayer.dependencies.ScheduleController = function() {
    ...
    var fragmentsToLoad = 0, type, ready, fragmentModel, isDynamic, currentTrackInfo, initialPlayback = true, lastValidationTime = null, isStopped = false, playListMetrics = null, playListTraceMetrics = null, playListTraceMetricsClosed = true, clearPlayListTraceMetrics = function(endTime, stopreason) {
    ...
    }, onStreamCompleted = function(sender, model) {
        if (model !== this.streamProcessor.getFragmentModel()) return;
        this.debug.log(type + " Stream is complete.");
        clearPlayListTraceMetrics(new Date(), MediaPlayer.vo.metrics.PlayList.Trace.END_OF_CONTENT_STOP_REASON);
    }, onMediaFragmentLoadingStart = function(sender, model) {
        var self = this;
        if (model !== self.streamProcessor.getFragmentModel()) return;
        validate.call(self);
        ...
        var self = this, req = getInitRequest.call(self, quality);
        ...
        var currentTime, metrics = this.metricsModel.getMetricsFor("stream"), manifestUpdateInfo = this.metricsExt.getCurrentManifestUpdate(metrics);
...
MediaPlayer.dependencies.Stream = function() {
    "use strict";
    var manifest, mediaSource, mediaInfos = {}, streamProcessors = [], autoPlay = true, initialized = false, loaded = false, errored = false, kid = null, initData = [], updating = true, streamInfo = null, needKeyListener, keyMessageListener, keyAddedListener, keyErrorListener, eventController = null, play = function() {
        if (!initialized) {
            return;
        }
        ...
        var self = this, mediaInfo = mediaInfos.video, videoCodec = mediaInfos ? mediaInfos.video.codec : null;
        initData.push({
            type: event.type,
        ...
        if (mediaInfo && !!videoCodec && !kid) {
            try {
                kid = self.protectionController.selectKeySystem(mediaInfo, event.initData);
            } catch (error) {
                pause.call(self);
        ...
        self.protectionController.updateFromMessage(kid, session, sessionId, event.message, laURL);
        ...
        var self = this, ln = streamProcessors.length, i = 0;
    ...
    }, onDataUpdateCompleted = function() {
...
MediaPlayer.dependencies.TextController = function() {
    ...
    }, onInitFragmentLoaded = function(sender, model, bytes) {
        var self = this;
        if (model !== self.streamProcessor.getFragmentModel()) return;
        if (bytes !== null) {
...
MediaPlayer.rules.PlaybackTimeRule = function() {
    "use strict";
    var seekTarget = {}, scheduleController = {}, onPlaybackSeeking = function(sender, time) {
